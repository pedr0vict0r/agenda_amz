...
@Test
@WithMockUser(roles = "EDITOR")
void naoDeveSalvarAlteracaoComContatoInexistente() throws Exception {
when(contatoRepository.findByCodigo(999L)).thenReturn(null);

mockMvc.perform(post("/contatos/999/editar")
.with(csrf())
.param("codigo", "999")
.param("nome", "Carlos")
.param("numero", "(41) 96666-5555"))
...
mockMvc.perform(post("/contatos/4/excluir").with(csrf()))
.andExpect(status().is3xxRedirection())
.andExpect(redirectedUrl("/contatos"));
...
@Test
@WithMockUser(roles = "EDITOR")
void naoDeveExcluirContatoInexistente() throws Exception {
when(contatoRepository.findByCodigo(eq(404L))).thenReturn(null);

mockMvc.perform(post("/contatos/404/excluir").with(csrf()))
.andExpect(status().is3xxRedirection())
.andExpect(redirectedUrl("/contatos"));
